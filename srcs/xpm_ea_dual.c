...
int	xpm_ea_dual(t_game *game)
{
	...
	i_tab = 0;
	i_color = -1;
	if (xpm_ea_init_color(game) == FAIL)
		return (FAIL);
	while (++i_color < game->xpm->ea_metadata[2])
	{
		i_chr = 2;
		if (xpm_ea_check_dual_colors(game, &i_chr, ++i_tab, i_color) == FAIL)
			return (FAIL);
		if (xpm_ea_hex_to_dec_dual(game, i_color, i_tab, i_chr) == FAIL)
			return (FAIL);
	}
	(void)game;
	return (SUCCESS);
...
int	xpm_ea_check_dual_colors(t_game *g, int *i_chr, int i_tab_file, int i_color)
{
	if (is_space(g->xpm->ea_tab_file[i_tab_file][(*i_chr)]) == FAIL)
		return (printf("1 dual ea something wrong xpm files\n"), FAIL);
	while (is_space(g->xpm->ea_tab_file[i_tab_file][(*i_chr)]) == SUCCESS)
		(*i_chr)++;
	if (g->xpm->ea_tab_file[i_tab_file][(*i_chr)] != 'c')
		return (printf(" 2 dual ea something wrong xpm files\n"), FAIL);
	(*i_chr)++;
	if (is_space(g->xpm->ea_tab_file[i_tab_file][(*i_chr)]) == FAIL)
		return (printf(" 3 dual ea something wrong xpm files\n"), FAIL);
	while (is_space(g->xpm->ea_tab_file[i_tab_file][(*i_chr)]) == SUCCESS)
		(*i_chr)++;
	g->xpm->ea_colors[i_color] = malloc(sizeof(int) * (5));
	if (!g->xpm->ea_colors[i_color])
		return (FAIL);
	g->xpm->ea_colors[i_color][0] = g->xpm->ea_tab_file[i_tab_file][0];
	g->xpm->ea_colors[i_color][1] = g->xpm->ea_tab_file[i_tab_file][1];
	return (SUCCESS);
}
...
int	xpm_ea_hex_to_dec_dual(t_game *g, int i_color, int i_tab_file, int i_chr)
{
	if (g->xpm->ea_tab_file[i_tab_file][i_chr] == '#')
	{
		...
		hex_to_dec(ft_strdup_len(g->xpm->ea_tab_file[i_tab_file], \
		i_chr + 3, i_chr + 5));
		g->xpm->ea_colors[i_color][4] = \
		hex_to_dec(ft_strdup_len(g->xpm->ea_tab_file[i_tab_file], \
		i_chr + 5, i_chr + 7));
	}
	else if (xpm_ea_dual_letter(g, i_color, i_tab_file, i_chr) == FAIL)
		return (FAIL);
	return (SUCCESS);
}
...
int	xpm_ea_dual_letter(t_game *game, int i_color, int i_tab_file, int i_chr)
{
	...
	if (ft_strncmp(tmp, "white", ft_strlen(tmp)) == 0)
	{
		...
		game->xpm->ea_colors[i_color][4] = 255;
	}
	...
	|| (ft_strncmp(tmp, "none", ft_strlen(tmp)) == 0))
	{
		...
		game->xpm->ea_colors[i_color][4] = 0;
	}
	else if (xpm_ea_set_dual_color(game, i_color, tmp) == FAIL)
...
int	xpm_ea_set_dual_color(t_game *game, int i_color, char *tmp)
{
	...
	else if (ft_strncmp(tmp, "blue", ft_strlen(tmp)) == 0)
	{
		...
		return (printf("ea color dual not handle\n"), FAIL);
	return (SUCCESS);
}
