...
/*   Updated: 2023/08/07 11:31:28 by gael             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	xpm_no_set_color(t_game *game, int i_color, char *tmp)
{
	...
	else if (ft_strncmp(tmp, "blue", ft_strlen(tmp)) == 0)
	{
		...
		return (printf("color no not handle\n"), FAIL);
	return (SUCCESS);
}
...
int	xpm_no_hex_to_dec(t_game *g, int i_color, int i_tab_file, int i_chr)
{
	...
	else if (xpm_no_letter_color(g, i_color, i_tab_file, i_chr) == FAIL)
		return (FAIL);
	return (SUCCESS);
}
...
int	xpm_no_set_len_n_color(t_game *g, char **line)
{
	...
	i_tab = 0;
	i_color = -1;
	xpm_no_fill_metadata(g, line);
	if (ft_atoi(line[2]) <= 92)
	{
		if (xpm_no_init_color(g) == FAIL)
			return (FAIL);
		while (++i_color < g->xpm->no_metadata[2])
		{
			i_chr = 1;
			if (xpm_no_check_line_color(g, &i_chr, ++i_tab, i_color) == FAIL)
				return (FAIL);
			if (xpm_no_hex_to_dec(g, i_color, i_tab, i_chr) == FAIL)
				return (FAIL);
		}
	}
	else
